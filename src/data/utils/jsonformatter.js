...
var jsonData = require("../tobaccouse/" + filename);
...
jsonData.forEach(each => {
  var maleFormat = parseFloat(each.Male.split(" ")[0]);
  var femaleFormat = parseFloat(each.Female.split(" ")[0]);
...
fs.writeFile("../tobaccouse/" + outputname, jsonContent, "utf8", function(err) {
  if (err) {
